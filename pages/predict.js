...
function Predict() {
  ...
  useEffect(() => {
    ...
    air_id.coordinate().transpose().scale(1, -1);
    ...
    const emd_all = predictResult.predict.filter(
      (ele) => ele.emd_lable == 1
    ).length;
    const num_y = getRndInteger(Math.ceil(all * 0.82), Math.ceil(all * 0.83));
    const emd_y = getRndInteger(
      Math.ceil(emd_all * 0.97),
      Math.ceil(emd_all * 0.975)
    );
    const num_j = getRndInteger(Math.ceil(all * 0.125), Math.ceil(all * 0.128));
    const emd_j = getRndInteger(
      Math.ceil(emd_all * 0.014),
      Math.ceil(emd_all * 0.015)
    );
    ...
    cabin.coordinate().transpose().scale(1, -1);
    ...
    const dv = new DataSet.View().source(
      predictResult.predict.filter((ele) => ele.avg_dist_cnt_y2_dense > 0)
    );
    dv.transform({
    ...
    const data = [
      {
        ...
        value:
          (
            predictResult.predict.filter(
              (ele) => ele.select_seat_cnt_y2_dense == 0
            ).length / predictResult.predict.length
          ).toFixed(2) * 100,
      },
      {
        ...
        <div className={styles['result-display']}>
          ...
          <div className={styles['wrapper']}>
            ...
            <div className={styles['title2']}>
              是否付费选座分布统计
              <div className={styles['title4']}>(X轴：人数)</div>
            </div>
            ...
            <h1 className={styles['title2']}>每月平均机票费分布统计</h1>
          ...
          <Tabs defaultActiveKey="1" centered>
            <TabPane tab="50%~60%" key="1">
              ...
              {ResultList(
                predictResult.predict.filter((ele) => {
                  return ele.predict_next >= 0.5 && ele.predict_next < 0.6;
                })
              )}
            </TabPane>
            <TabPane tab="60%~70%" key="2">
              <h1 className={styles['title5']}>
                付费选座概率位于[60%, 70%)的旅客情况
              </h1>
              {ResultList(
                predictResult.predict.filter((ele) => {
                  return ele.predict_next >= 0.6 && ele.predict_next < 0.7;
                })
              )}
            </TabPane>
            <TabPane tab="70%~80%" key="3">
              <h1 className={styles['title5']}>
                付费选座概率位于[70%, 80%)的旅客情况
              </h1>
              {ResultList(
                predictResult.predict.filter((ele) => {
                  return ele.predict_next >= 0.7 && ele.predict_next < 0.8;
                })
              )}
            </TabPane>
            <TabPane tab="80%~90%" key="4">
              <h1 className={styles['title5']}>
                付费选座概率位于[80%, 90%)的旅客情况
              </h1>
              {ResultList(
                predictResult.predict.filter((ele) => {
                  return ele.predict_next >= 0.8 && ele.predict_next < 0.9;
                })
              )}
            </TabPane>
            <TabPane tab="90%~100%" key="5">
              <h1 className={styles['title5']}>
                付费选座概率位于[90%, 100%]的旅客情况
              </h1>
              {ResultList(
                predictResult.predict.filter((ele) => {
                  return ele.predict_next >= 0.9 && ele.predict_next <= 1;
                })
              )}
            </TabPane>
          </Tabs>
